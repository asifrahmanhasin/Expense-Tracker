...
<body>
  <div class="app">
    <header>
      ...
      <div>
        ...
        <p class="lead">Track any month — mobile first, export to Excel</p>
      </div>
    </header>
    ...
    <div class="card" id="appCard">
      ...
      <div class="summary" style="margin-top:10px">
        <div>
          ...
          <div id="selectedMonth" class="pill">—</div>
        </div>
  ...
  <script>
    ...
    function loadEntries(){
      ...
      selectedMonthEl.textContent = monthInput.value || '—';
    ...
    function render(entries){
      ...
      entries.forEach((it,i)=>{
        ...
        const cat = document.createElement('div'); cat.className='cat'; cat.textContent = `${it.category} — ${fmtMoney(it.amount)}`;
        const descEl = document.createElement('div'); descEl.className='desc'; descEl.textContent = `${it.date} ${it.desc? ' · '+it.desc:''}`;
        meta.appendChild(cat); meta.appendChild(descEl);
        const controls = document.createElement('div'); controls.className='controls';
        const del = document.createElement('button'); del.className='icon-btn'; del.title='Delete'; del.innerHTML='🗑️'; del.onclick = ()=>{ if(confirm('Delete this entry?')) removeEntry(i)};
